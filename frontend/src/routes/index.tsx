    ...
    <div className="group relative flex items-center justify-between gap-4 px-4 py-3 rounded-xl border border-white/5 bg-bg-1/80 backdrop-blur-[1px] hover:border-white/15 hover:bg-bg-2/90 hover:shadow-[0_4px_12px_rgba(0,0,0,0.3)] transition-all duration-200 ease-out hover:-translate-y-0.5">
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {poll.status !== "DRAFT" && (
          ...
          >
            {copied ? (
              ...
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="3" y="2" width="8" height="4" rx="1" stroke="currentColor" strokeWidth="1.2" fill="none"/>
                <path d="M4 6h6v5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6z" stroke="currentColor" strokeWidth="1.2" fill="none"/>
              </svg>
            )}
        ...
        {poll.status !== "DRAFT" && (
          ...
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M6 2H2a1 1 0 0 0-1 1v9a1 1 0 0 0 1 1h9a1 1 0 0 0 1-1V8M9 1h4m0 0v4m0-4L6 8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/>
            </svg>
          </Link>
        )}
      </div>
  ...
  return (
    <>
      <TopBar
        ...
        right={
          <Link to="/create">
            <Button variant="accent" size="sm" className="shadow-sm hover:shadow-md transition-shadow">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
        <div className="mb-7 flex items-baseline justify-between">
          ...
          <p className="text-[11px] text-ink-2 tabular-nums">{polls.length} total</p>
        </div>

        {polls.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4 animate-fade-in-up">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-ink-2/10 to-ink-3/5 flex items-center justify-center">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" className="text-ink-2">
                <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" strokeLinecap="round" />
                <circle cx="12" cy="12" r="3" />
              </svg>
            </div>
            <p className="text-[14px] text-ink-1">No polls yet</p>
            <p className="text-[12px] text-ink-2 max-w-[220px]">Create your first poll and share it with the world.</p>
            <Link to="/create">
              <Button variant="accent" className="mt-2">Create poll →</Button>
            </Link>
          </div>
        ) : (
